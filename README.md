...
- **The ghost** — a little site pet, ported from [vault](https://github.com/Real-Fruit-Snacks/vault). It drifts, peeks at your cards, naps, and flees the cursor — and it plays on the map: sailing the oceans with a wake, hopping city to city, surfing the day/night line, and dozing on the sun. Sometimes it chases your cursor for a bit; spam-boop it and it goes dizzy; it leaves faint afterimages when it bolts. Click it to recolor it. Cursor-follow and off modes, size/opacity sliders, and per-quirk toggles (including an opt-in jump scare) live in settings.
